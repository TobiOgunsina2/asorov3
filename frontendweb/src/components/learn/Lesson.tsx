...
const Lesson = ({isSelected, lesson}: Props) => {
    
    

    const router = useRouter();
  const goToLesson = () => {
    router.push(`unit/1/lessons/${lesson.code}`); // or slug
  };

  return isSelected ? (
    <div className="bg-linear-to-br from-ewe-300/90 to-ewe-500/90 backdrop-blur-md h-full w-full rounded-3xl flex flex-row items-center justify-evenly hover:shadow-xl">
      ...
      <div className="lesson-info rounded-2xl w-7/16 h-48 px-0.5 text-center flex flex-col items-center">
        <h1 className="lesson-title text-md text-white font-bold">{lesson.title}</h1>
        <h4 className="lesson-description text-xs font-bold text-gray-300">{lesson.description}</h4>
        <button
          onClick={(e) => {
            e.stopPropagation();
            goToLesson();
